    ...
    function startTimers(){
        ...
        warnTimer = setTimeout(()=>{
            ...
            if (window.Swal){
                window.Swal.fire({
                    title:'Inactividad',
                    html:`Cierre de sesión en <b id="idleCountdown">5</b>s`,
        ...
        idleTimer = setTimeout(()=>{
            fetch('/api/auth/logout', { method:'POST', credentials:'include', keepalive:true }).finally(()=>{ window.location.href = '/login?error=inactive'; });
        }, LIMIT);
    }
    function reset(){
